...
def start_game():
    ...
    if btn.active_easy == True:
        ...
        dificulty = 'hard'
    game_settings(int(mn.text_width), int(mn.text_height), dificulty)
    gameloop()

def mouse_click( event):
    if btn.resolution_width.collidepoint(event.pos):
        btn.active_width = True
        btn.active_height = False
    elif btn.resolution_height.collidepoint(event.pos):
        btn.active_width = False
        btn.active_height = True
    elif btn.difficulty_box_easy.collidepoint(event.pos):
        btn.active_easy = True
        btn.active_med = False
        btn.active_hard = False
        btn.active_width = False
        btn.active_height = False
    elif btn.difficulty_box_med.collidepoint(event.pos):
        btn.active_easy = False
        btn.active_med = True
        btn.active_hard = False
        btn.active_width = False
        btn.active_height = False
    elif btn.difficulty_box_hard.collidepoint(event.pos):
        btn.active_easy = False
        btn.active_med = False
...
def buttons_text():
    ...
    # Blit the text.
    mn.screen.blit(txt_width, (btn.resolution_width.x+5, btn.resolution_width.y+5))
    mn.screen.blit(txt_height, (btn.resolution_height.x+5, btn.resolution_height.y+5))
    mn.screen.blit(txt_easy, (btn.difficulty_box_easy.x+5, btn.difficulty_box_easy.y+5))
    mn.screen.blit(txt_med, (btn.difficulty_box_med.x+5, btn.difficulty_box_med.y+5))
    mn.screen.blit(txt_hard, (btn.difficulty_box_hard.x+5, btn.difficulty_box_hard.y+5))
    mn.screen.blit(txt_Start, (btn.Start_box.x+5, btn.Start_box.y+5))
    mn.screen.blit(txt_Quit, (btn.Quit_box.x+5, btn.Quit_box.y+5))

def render_button():
    pg.draw.rect(mn.screen, btn.color_width, btn.resolution_width, 2)
    pg.draw.rect(mn.screen, btn.color_height, btn.resolution_height, 2)
    pg.draw.rect(mn.screen, btn.active_easy, btn.difficulty_box_easy, 2)
    pg.draw.rect(mn.screen, btn.active_med, btn.difficulty_box_med, 2)
    pg.draw.rect(mn.screen, btn.active_hard, btn.difficulty_box_hard, 2)
    pg.draw.rect(mn.screen, btn.color_inactive, btn.Start_box, 2)
    pg.draw.rect(mn.screen, btn.color_inactive, btn.Quit_box, 2)

def render_info():
    render_text_and_position('Hello world!', mn.font, (255, 255, 255), mn.screen, 300, 20)
    render_text_and_position('Choose resolution:', mn.font, (255, 255, 255), mn.screen, 150, 135)
    render_text_and_position('x', mn.font, btn.color_inactive, mn.screen, 408, 135)
    render_text_and_position('Choose difficulty:', mn.font, (255, 255, 255), mn.screen, 150, 200)
    render_text_and_position('Hello world!', mn.font, (255, 255, 255), mn.screen, 300, 20)
